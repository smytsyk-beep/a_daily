...
def compare_users(user_id_1: int, user_id_2: int):
    ...
    with session_scope() as db:
        ...
        print(
            f"User 1 digest length: {digest1.length} ({len(digest1.body)} chars, {len(digest1.body.split(chr(10)+chr(10)))} paragraphs)"
        )
        print(
            f"User 2 digest length: {digest2.length} ({len(digest2.body)} chars, {len(digest2.body.split(chr(10)+chr(10)))} paragraphs)"
        )
