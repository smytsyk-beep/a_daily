...
from app.daily_digest_service import build_daily_digest_for_user_id, make_user_profile_from_model
    ...
    for day in [yesterday, today]:
        ...
        if events:
            for i, event in enumerate(events, 1):
                print(f"  {i}. Event ID {event.id}: kind={event.kind}, title={event.title}")
                if event.details:
                    ...
                    print(f"     Details: trigger={trigger}, {transit_body} {aspect} {natal_body}")
        else:
        ...
        if selected_atoms:
            for i, sel in enumerate(selected_atoms, 1):
                atom = sel.atom
                print(f"  {i}. Atom ID {atom.id}: {atom.trigger or 'no trigger'} | {atom.topic_tag} | score={sel.score:.2f}")
                if sel.event:
