...
from app.daily_digest_service import build_daily_digest_for_user, make_user_profile_from_model
...
with session_scope() as db:
    ...
    all_ru_atoms = db.query(models.ContentAtom).filter(
    ...
    for atom in all_ru_atoms:
        has_cyrillic = 'Сегодня' in atom.body or any(ord(c) >= 1024 for c in atom.body[:50])
        has_questions = atom.body.count('?') > len(atom.body) * 0.3
        
        if has_questions and not has_cyrillic:
            broken_count += 1
            print(f"❌ BROKEN: ID={atom.id}, trigger={atom.trigger}, topic={atom.topic_tag}")
            print(f"   Body: {atom.body[:80]}...")
        else:
            good_count += 1
            if good_count <= 3:  # Show first 3 good ones
                print(f"✅ GOOD: ID={atom.id}, trigger={atom.trigger}, topic={atom.topic_tag}")
                print(f"   Body: {atom.body[:80]}...")
    ...
    for i, sel in enumerate(selected, 1):
        atom = sel.atom
        has_cyrillic = any(ord(c) >= 1024 for c in atom.body[:50]) if atom.body else False
        has_questions = atom.body.count('?') > len(atom.body) * 0.3 if atom.body else False
