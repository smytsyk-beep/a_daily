...
with session_scope() as db:
    # Check atoms count
    ru_atoms = (
        db.query(models.ContentAtom).filter(models.ContentAtom.locale == "ru").count()
    )
    en_atoms = (
        db.query(models.ContentAtom).filter(models.ContentAtom.locale == "en").count()
    )
    ...
    if ru_atom:
        ...
        print(
            f"  Valid UTF-8: {all(c == ' ' or ord(c) >= 32 for c in ru_atom.body[:100])}"
        )
        print()
